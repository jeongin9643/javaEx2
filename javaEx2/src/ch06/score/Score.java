...
public class Score {
	...
	public Score() {// 생성자
		list = new ScoreVO[100]; // ScoreVO 객체를 100개 저장할 수 있는 배열. 배열은 한 번 정하면 못 바꿈
	}
	...
	public int append(ScoreVO vo) {
		...
		list[count++] = vo; // 배열에 학생 정보(vo) 저장. list 0번째 방에 vo를 넣은 후 count +1
	...
	public void deleteScore(int index) { // list배열에서 index 위치의 자료를 삭제
		...
		for(int i =index; i<count-1; i++) {  // i =2; i<4; i++ --> 2번 댕김.
